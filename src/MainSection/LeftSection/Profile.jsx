...
import { Sparkles, Sun, Moon, PenSquare, FileText, Users, UserCheck } from "lucide-react";
...
const Profile = () => {
  ...
  // ─── Color scheme matching SideBar & Feed components ──────────────────────
    ...
    >
      ...
      >
        ...
        <div className="px-6 sm:px-8 pt-4 pb-2 text-center">
          <h1 className="text-2xl sm:text-3xl font-bold" style={{ color: textMain }}>
            {User?.firstName} {User?.lastName}
          </h1>

          <p className="text-xs flex items-center justify-center gap-1 mt-1" style={{ color: textSub }}>
            <Sparkles className="w-3 h-3" style={{ color: accentColor }} />
            Active now
          </p>

          <p className="text-sm sm:text-base mt-3 leading-relaxed" style={{ color: textSub }}>
            {User?.about || "No bio available"}
          </p>
        </div>

        {/* ── Divider ── */}
        <div className="mx-6 sm:mx-8 my-4 h-px" style={{ backgroundColor: border }} />
        ...
        {/* ── EDIT PROFILE BUTTON ── */}
        <div className="flex justify-center px-6 sm:px-8 pt-6 pb-8">
          <Link to="/EditProfile">
            <button
              className="relative flex items-center gap-2 px-8 py-3 cursor-pointer font-bold rounded-2xl text-white transition-all duration-300 hover:scale-105 active:scale-95 overflow-hidden group"
              style={{
                background: `linear-gradient(135deg, ${accentColor} 0%, #CC006D 100%)`,
            ...
            >
              ...
              <span className="relative z-10 text-sm sm:text-base">Edit Profile</span>
            </button>
          </Link>
        </div>
      </div>
